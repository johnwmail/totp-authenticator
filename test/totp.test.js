...
async function runTests() {
    ...
    await runImportExportTest();
    await runLoadAccountsJsonTest();
    await runDemoAccountTest();
...
async function runImportExportTest() {
    ...
    globalThis.URL = {
        createObjectURL: function(blob) { exportedBlob = blob; return 'blob:'; }
    };

    try {
        assert(true, 'Export function exists');
    } finally {
        globalThis.document = originalDocument;
        globalThis.Storage = originalStorage;
        globalThis.localStorage = originalLocalStorage;
...
async function runLoadAccountsJsonTest() {
    console.log('load accounts.json on first visit:');
    ...
    globalThis.fetch = async function(url) {
        ...
        if (url === 'accounts.json') {
            return {
                ok: true,
                text: async () => '[{"name":"test","secret":"JBSWY3DPEHPK3PXP","issuer":"Test"}]'
            };
        }
    ...
    try {
        ...
        assert(fetchCalled, 'fetches accounts.json on first visit with empty storage');

        const accounts = JSON.parse(store.get('accounts') || '[]');
        assert(accounts.length === 1, 'loads account from accounts.json');
        assert(accounts[0].name === 'test', 'parses account name from accounts.json');
    } finally {
        globalThis.document = originalDocument;
        globalThis.Storage = originalStorage;
        globalThis.localStorage = originalLocalStorage;
...
async function runDemoAccountTest() {
    console.log('show demo account if no accounts.json:');
    ...
    try {
        ...
        const accounts = JSON.parse(store.get('accounts') || '[]');
        assert(accounts.length >= 1, 'shows demo account when offline and no accounts.json');
        assert(accounts[0].issuer === 'Github', 'demo account has Github issuer');
    } finally {
        globalThis.document = originalDocument;
        globalThis.Storage = originalStorage;
        globalThis.localStorage = originalLocalStorage;
